...
def normalize_department(value):
    """統一開課單位為「中文名稱（代號）」並移除英文全名。"""
    value = clean(value)
    if not value:
        return ''
    match = re.match(r'^([A-Z0-9]+)[.．]\s*(.+)$', value)
    if not match:
        return value
    code, label = match.groups()
    # 115 學期來源會在中文名稱後附英文全名；從第一個英文字母片段起移除。
    label = re.split(r'\s+(?=[A-Za-z])', label, maxsplit=1)[0].strip()
...
def parse_file(path, semester='1151CLASS'):
    ...
    for table in soup.find_all('table'):
        ...
        if first.startswith('系別(Department)：'):
            current_department = normalize_department(first.split('：', 1)[1])
