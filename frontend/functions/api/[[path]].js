...
async function handleAdminPatchCourseImport(request, env, sql) {
  ...
  const repairAcademicYear114 = Boolean(body.repairAcademicYear114 || body.repair_academic_year_114 || body.rebuild114 || body.rebuild_114)
  ...
  if (repairAcademicYear114) {
    // 114 學年重建採乾淨重建策略：先清除 1141/1142，再以內建靜態課表與補丁完整寫回。
    // 這比用 serial/code 猜測錯置補丁更安全，可避免英文、國防、共同課因同課號同班別互吞或殘留錯學期。
  ...
  return json({
    ...
    patchVersion: '114-academic-year-v3',
  })
}
...
function courseDepartmentMatches(course, department) {
  const target = normalizeCourseFilterText(department)
  if (!target || target === normalizeCourseFilterText('全部')) return true
  return courseDepartmentTexts(course).some((item) => {
    const text = normalizeCourseFilterText(item)
...
function courseMatchesQuery(course, searchParams) {
  ...
  if (!courseDepartmentMatches(course, department)) return false
