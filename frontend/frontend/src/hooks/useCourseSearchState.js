...
export function useCourseSearchState({ activeSemester, favorites = [], candidates = [], plan = [], tagVotes = {} }) {
  ...
  const sortedFilteredCourses = useMemo(() => {
    ...
    const list = indexedCourses.filter((record) => {
      ...
      if (!parsedQuery.department && searchFilters.department && searchFilters.department !== '全部') {
        const selectedDepartmentPhonetic = toPhoneticKey(searchFilters.department)
      ...
      if (query && !courseRecordMatchesParsed(record, parsedQuery)) return false
      return true
    })
    ...
    if (tagFilter !== '全部') {
      ...
      }).slice(0, visibleCourseLimit(query)).map((record) => record.course)
    }
    ...
    if (searchSort === 'credits' || searchSort === 'creditsDesc') return list.sort((a, b) => credits(b.course) - credits(a.course)).slice(0, visibleCourseLimit(query)).map((record) => record.course)
    if (searchSort === 'name') return list.sort((a, b) => a.nameSort.localeCompare(b.nameSort, 'zh-Hant')).slice(0, visibleCourseLimit(query)).map((record) => record.course)
    if (searchSort === 'favorite') return list.sort((a, b) => Number(favoriteKeys.has(b.courseKey)) - Number(favoriteKeys.has(a.courseKey))).slice(0, visibleCourseLimit(query)).map((record) => record.course)
    if (searchSort === 'candidate') return list.sort((a, b) => Number(candidateKeys.has(b.courseKey)) - Number(candidateKeys.has(a.courseKey))).slice(0, visibleCourseLimit(query)).map((record) => record.course)
    return list.sort((a, b) => {
  ...
  }, [indexedCourses, activeSemester, searchOnlyAvailable, searchSort, favoriteKeys, candidateKeys, plan, searchFilters.tag, searchFilters.department, searchFilters.requirement, searchFilters.grade, tagVotes, query, departmentPool])
