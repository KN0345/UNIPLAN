...
export default function AdminDataConsole({ notify, courses = [], user, profile, plan = {}, candidates = [], favorites = [] }) {
  ...
  const [selectedPatchAction, setSelectedPatchAction] = useState('latest')
  ...
  const FRONTEND_PATCH_ACTIONS = [
    {
      key: 'latest',
      label: '通用內建補丁',
      description: '寫入目前前端內建的缺失課程與資料修補。未來新增補丁預設放這裡。',
      payload: () => ({
        action: 'latest',
        semester: importSemester,
        clearSemester: false,
      }),
      done: (result) => `已執行通用內建補丁：匯入 ${result.imported || 0} 筆`,
    },
    {
      key: 'repair1141',
      label: '修復 114上英文/共同課補丁',
      description: '只處理 1141CLASS 既有補丁與錯置清理。',
      payload: () => ({
        action: 'repair1141',
        semester: '1141CLASS',
        onlySemester: true,
      ...
      done: (result) => `已修復 114上補丁：匯入 ${result.imported || 0} 筆，清除錯置 ${result.repairedWrongSemester || 0} 筆`,
    },
    {
      key: 'rebuild114',
      label: '重建 114學年補丁',
      description: '重建 1141CLASS / 1142CLASS 的內建補丁資料。',
      payload: () => ({
        action: 'rebuild114',
        repairAcademicYear114: true,
      }),
      done: (result) => `已重建 114學年補丁：匯入 ${result.imported || 0} 筆，清除錯置 ${result.repairedWrongSemester || 0} 筆`,
    },
  ]

  async function runSelectedFrontendPatch() {
    const action = FRONTEND_PATCH_ACTIONS.find((item) => item.key === selectedPatchAction) || FRONTEND_PATCH_ACTIONS[0]
    setPatchImportBusy(true)
    setImportResult(null)
    try {
      const result = await importPatchedCourses(action.payload())
      setImportResult({ ...result, patchAction: action.label })
      notify?.(action.done(result))
    } catch (error) {
      const message = error?.response?.data?.error || error?.response?.data?.message || error?.message || `${action.label} 執行失敗`
      setImportResult({ ok: false, error: message, patchAction: action.label })
      notify?.(message)
    } finally {
        ...
        <div className="adminConsoleContent">
          ...
          {activeTab === 'courseImport' && <section className="adminSection adminImportSection">
            ...
            <div className="adminImportActions">
              ...
              <div className="adminPatchUnifiedControl">
                <label>前端補丁
                  <select value={selectedPatchAction} onChange={(event) => setSelectedPatchAction(event.target.value)}>
                    {FRONTEND_PATCH_ACTIONS.map((action) => <option key={action.key} value={action.key}>{action.label}</option>)}
                  </select>
                </label>
                <p>{FRONTEND_PATCH_ACTIONS.find((action) => action.key === selectedPatchAction)?.description}</p>
                <button type="button" disabled={patchImportBusy} onClick={runSelectedFrontendPatch}>{patchImportBusy ? '執行中…' : '執行前端補丁'}</button>
              </div>
