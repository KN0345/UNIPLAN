...
export default function AdminDataConsole({ notify, courses = [], user, profile, plan = {}, candidates = [], favorites = [] }) {
  ...
  const courseAuditRows = useMemo(() => {
    ...
    return adminCourses.filter((course) => normalizeText(`${course.code} ${course.id} ${course.name} ${course.teacher} ${course.department} ${course.semesterSource}`).includes(q)).slice(0, 240)
