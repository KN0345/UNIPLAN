...
export default function AdminDataConsole({ notify, courses = [], user, profile, plan = {}, candidates = [], favorites = [] }) {
  ...
  async function repairGrade1141PatchCourses() {
    setPatchImportBusy(true)
    setImportResult(null)
    try {
      const result = await importPatchedCourses({
      ...
      setImportResult(result)
      notify?.(`已修復 114上補丁：匯入 ${result.imported || 0} 筆，清除錯置 ${result.repairedWrongSemester || 0} 筆`)
    } catch (error) {
      const message = error?.response?.data?.error || error?.message || '114上補丁修復失敗'
      setImportResult({ ok: false, error: message })
      notify?.(message)
    } finally {
      setPatchImportBusy(false)
  ...
  async function rebuildGrade114PatchCourses() {
    setPatchImportBusy(true)
    setImportResult(null)
    try {
      const result = await importPatchedCourses({
      ...
      setImportResult(result)
      notify?.(`已重建 114學年補丁：匯入 ${result.imported || 0} 筆，清除錯置 ${result.repairedWrongSemester || 0} 筆`)
    } catch (error) {
      const message = error?.response?.data?.error || error?.message || '114學年補丁重建失敗'
      setImportResult({ ok: false, error: message })
      notify?.(message)
    } finally {
      setPatchImportBusy(false)
        ...
        <div className="adminConsoleContent">
          ...
          {activeTab === 'courseImport' && <section className="adminSection adminImportSection">
            ...
            <div className="adminImportActions">
              ...
              <button type="button" disabled={patchImportBusy} onClick={repairGrade1141PatchCourses}>{patchImportBusy ? '修復中…' : '修復 114上英文/共同課補丁'}</button>
              <button type="button" disabled={patchImportBusy} onClick={rebuildGrade114PatchCourses}>{patchImportBusy ? '重建中…' : '重建 114學年補丁'}</button>
