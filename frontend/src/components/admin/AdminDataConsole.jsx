...
function normalizeAdminCourse(course, index = 0) {
  ...
  return {
    ...
    academicYear: c.academic_year || c.year || c.entryYear || c.school_year || c.semester_source || c.semester || '',
    semesterSource: c.semester_source || c.semester || c.academic_year || c.year || '',
...
const COURSE_MISSING_CHECK_PRESETS = [
  ['TGAXM', '文學院共同科－碩'],
  ['TGDXM', '教育學院共同科－碩'],
  ['TGEXM', '工學院共同科－碩'],
  ['TGKXM', '創智學院共同科－碩'],
...
const DEPARTMENT_ALIAS_PRESETS = [
  ['教科系', '教育科技學系 / 教育科技系 / 教科 / 教育科系學系'],
  ['資工系', '資訊工程學系 / 資訊工程系 / 資工'],
  ['資管系', '資訊管理學系 / 資訊管理系 / 資管'],
  ['企管系', '企業管理學系 / 企業管理系 / 企管'],
  ['中文系', '中國文學學系 / 中國文學系 / 中文'],
  ['商管共同科', '商管學院共同科 / TGLXB / TGLXM'],
  ['教育共同科', '教育學院共同科 / TGDXB / TGDXM'],
  ['文學共同科', '文學院共同科 / TGAXB / TGAXM'],
  ['工學共同科', '工學院共同科 / TGEXB / TGEXM'],
  ['創智共同科', 'AI學院共同科 / 創智學院共同科 / TGKXB / TGKXM'],
]

function auditSearchBlob(course) {
  return normalizeText(`${course.code} ${course.id} ${course.name} ${course.teacher} ${course.department} ${course.semesterSource} ${course.academicYear}`)
}
...
export default function AdminDataConsole({ notify, courses = [], user, profile, plan = {}, candidates = [], favorites = [] }) {
  ...
  const [courseAuditQuery, setCourseAuditQuery] = useState('')
  const [courseMissingCheckQuery, setCourseMissingCheckQuery] = useState('')
  const [staticAuditCourses, setStaticAuditCourses] = useState([])
  ...
  useEffect(() => {
    ...
    fetch('/data/courses.json')
      ...
      .then((payload) => {
        ...
        setStaticAuditCourses(Array.isArray(list) ? list : [])
      })
      .catch(() => setStaticAuditCourses([]))
    return () => { cancelled = true }
  }, [])

  const adminCourses = useMemo(() => {
    const base = staticAuditCourses.length ? staticAuditCourses : (courses.length ? courses : [...effectiveCourses(plan), ...candidates, ...favorites])
    const seen = new Set()
    return base.map((course, index) => normalizeAdminCourse(course, index)).filter((course) => {
      const key = `${course.semesterSource || course.academicYear || ''}-${course.code || ''}-${course.id || ''}-${course.name}-${course.teacher}-${course.time}`
      if (seen.has(key)) return false
      seen.add(key)
      return true
    })
  }, [staticAuditCourses, courses, plan, candidates, favorites])
  ...
  const courseAuditStats = useMemo(() => {
    const termOf = (course) => String(course.semesterSource || course.academicYear || '').toUpperCase()
    const departmentCounts = new Map()
    adminCourses.forEach((course) => {
      const dept = course.department || '未標示系所'
    ...
    return {
      ...
      term1141: adminCourses.filter((course) => termOf(course).includes('1141')).length,
      term1142: adminCourses.filter((course) => termOf(course).includes('1142')).length,
      departmentTotal: departmentCounts.size,
      topDepartments,
    }
  }, [adminCourses])

  const courseAuditRows = useMemo(() => {
    const q = normalizeText(courseAuditQuery)
    if (!q) return adminCourses.slice(0, 120)
    return adminCourses.filter((course) => auditSearchBlob(course).includes(q)).slice(0, 240)
  }, [adminCourses, courseAuditQuery])

  const courseMissingCheckRows = useMemo(() => {
    const q = normalizeText(courseMissingCheckQuery)
    const targetPresets = COURSE_MISSING_CHECK_PRESETS.filter(([code, label]) => {
      if (!q) return true
      return normalizeText(`${code} ${label}`).includes(q)
    })
    return targetPresets.map(([code, label]) => {
      const normalizedCode = normalizeText(code)
      const normalizedLabel = normalizeText(label)
      const matches = adminCourses.filter((course) => {
        const blob = auditSearchBlob(course)
        return blob.includes(normalizedCode) || blob.includes(normalizedLabel)
      })
      return {
        code,
        label,
  ...
  }, [adminCourses, courseMissingCheckQuery])

  const missingPresetCount = useMemo(() => courseMissingCheckRows.filter((item) => !item.count).length, [courseMissingCheckRows])
      ...
      <div className="adminConsoleLayout">
        ...
        <div className="adminConsoleContent">
          ...
          {activeTab === 'courseImport' && <section className="adminSection adminImportSection">
            ...
            {importRows.length ? <div className="adminImportPreview adminImportPreviewTable"><table><thead><tr><th>學期</th><th>課號</th><th>課名</th><th>教師</th><th>學分</th><th>時間</th><th>來源</th></tr></thead><tbody>{importRows.slice(0, 12).map((row, index) => <tr key={`${row.serial}-${row.name}-${index}`}><td>{row.semester_source || '—'}</td><td>{row.code || row.serial || '—'}</td><td>{row.name || '—'}</td><td>{row.teacher || '—'}</td><td>{row.credits || '—'}</td><td>{row.time_info || '—'}</td><td>{row.raw_json?.source_file || importFileName || '—'}</td></tr>)}</tbody></table>{importRows.length > 12 ? <p className="muted adminImportPreviewHint">僅顯示前 12 筆；完整資料會在確認匯入時送出。</p> : null}</div> : <p className="muted">尚未選擇匯入檔案。</p>}
            ...
            {importResult && <div className={importResult.ok === false ? 'adminImportResult error' : 'adminImportResult'}>
              {importResult.ok === false ? <p>{importResult.error}</p> : <>
                <div className="adminImportResultStats">
                  <article><b>{importResult.imported || importResult.upserted || 0}</b><span>新增 / 更新</span></article>
                  <article><b>{importResult.inserted || importResult.created || 0}</b><span>新增課程</span></article>
                  <article><b>{importResult.updated || importResult.overwritten || 0}</b><span>覆蓋課程</span></article>
                  <article><b>{importResult.failed || importResult.skipped || 0}</b><span>失敗 / 忽略</span></article>
                </div>
                <p>匯入完成。{importResult.repairedWrongSemester ? `已清除錯置 ${importResult.repairedWrongSemester} 筆。` : ''}{importResult.bySemester ? ` 學期分布：${Object.entries(importResult.bySemester).map(([term, count]) => `${term} ${count}`).join('、')}` : ''}</p>
              </>}
            </div>}
          </section>}
          ...
          {activeTab === 'stats' && <section className="adminSection adminStatsDashboard">
            <div className="adminDashboardOverview">
              <article><b>{courseAuditStats.total}</b><span>總課程數</span></article>
              <article><b>{courseAuditStats.term1141}</b><span>1141 課程數</span></article>
              <article><b>{courseAuditStats.term1142}</b><span>1142 課程數</span></article>
              <article><b>{courseAuditStats.departmentTotal}</b><span>科系 / 單位數</span></article>
            </div>
            <div className="adminCourseAuditPanel">
              <div className="adminCourseAuditHead"><div><h3>課程資料完整性檢查</h3><p className="muted">可用課號、課名、教師、科系快速確認資料是否缺漏。統計優先讀取 frontend/public/data/courses.json。</p></div><input value={courseAuditQuery} onChange={(event) => setCourseAuditQuery(event.target.value)} placeholder="搜尋課號、課名、教師、科系" /></div>
              <div className="adminCourseAuditGrid">
                <div className="adminCourseAuditTableWrap"><table className="adminCourseAuditTable"><thead><tr><th>學期</th><th>課號</th><th>課名</th><th>教師</th><th>科系</th></tr></thead><tbody>{courseAuditRows.map((course, index) => <tr key={`${course.id}-${course.code}-${index}`}><td>{course.semesterSource || course.academicYear || '—'}</td><td>{course.code || course.id || '—'}</td><td>{course.name}</td><td>{course.teacher || '—'}</td><td>{course.department || '—'}</td></tr>)}</tbody></table></div>
                <div className="adminDepartmentCountList"><b>各科系課程數</b>{courseAuditStats.topDepartments.map(([dept, count]) => <button type="button" key={dept} onClick={() => setCourseAuditQuery(dept)}><span>{dept}</span><strong>{count}</strong></button>)}</div>
              </div>
              <div className="adminAuditUtilityGrid">
                <div className="adminAuditUtilityPanel">
                  <div className="adminCourseAuditHead compact"><div><h3>課程資料缺漏檢查工具</h3><p className="muted">快速檢查共同科、榮譽學程等常見補丁課群是否存在；數量為 0 代表目前資料源可能缺漏。</p></div><input value={courseMissingCheckQuery} onChange={(event) => setCourseMissingCheckQuery(event.target.value)} placeholder="搜尋 TGDXM、商管、榮譽學程" /></div>
                  <div className="adminMissingCheckSummary"><b>{missingPresetCount}</b><span>組可能缺漏</span></div>
                  <div className="adminMissingCheckList">{courseMissingCheckRows.map((item) => <button type="button" key={item.code} className={item.count ? 'ok' : 'missing'} onClick={() => setCourseAuditQuery(item.code)}>
                    <span><b>{item.label}</b><small>{item.code}{item.sample[0] ? `｜例：${item.sample[0].name}` : '｜目前查無課程'}</small></span><strong>{item.count}</strong>
                  </button>)}</div>
                </div>
                <div className="adminAuditUtilityPanel">
                  <h3>科系篩選別名庫</h3>
                  <p className="muted">這些名稱會作為搜尋與篩選的對照參考，方便用完整系名、簡稱或代碼查找。</p>
                  <div className="adminAliasList">{DEPARTMENT_ALIAS_PRESETS.map(([name, aliases]) => <article key={name}><b>{name}</b><span>{aliases}</span></article>)}</div>
                </div>
              </div>
            </div>
