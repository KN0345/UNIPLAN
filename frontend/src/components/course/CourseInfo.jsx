...
function CourseInfo({ course, reviews = [], tagVotes = {}, onTagVote, userId = '', onAddReview, onUpdateReview, onDeleteReview }) {
  ...
  const cleanTeacher = cleanTeacherName(c.teacher || c.instructor) || '尚無資料'
  const courseDescription = String(c.description || c.note || '').trim()
  ...
  return (
    <section className="courseInfoSafe">
      ...
      <div className="courseInfoGrid">
        <article><span>教師</span><strong>{cleanTeacher}</strong></article>
      ...
      {programMemberships.length ? <div className="courseInfoBlock courseProgramBlock">
        <h3>對應學程</h3>
        <div className="courseProgramTags">{programMemberships.map((item) => <span key={`${item.program}-${item.group}`}><b>{item.program}</b><small>{item.group}</small></span>)}</div>
      </div> : null}
      {courseDescription ? <div className="courseInfoBlock"><h3>課程簡介</h3><p>{courseDescription}</p></div> : null}
