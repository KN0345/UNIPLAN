...
function setCourseDragPreview(event, course, variant = 'planned') {
  ...
  const detail = getCourse(course)
  const room = detail.classroom || detail.room || detail.location || ''
  const title = detail.name || '未命名課程'
  const meta = variant === 'candidate'
    ? [`${credits(course)} 學分`, detail.time_info || detail.time || '未列時間'].filter(Boolean).join('｜')
    : [scheduleRuleLabel(course), room || '未列教室'].filter(Boolean).join('｜')

  const span = Math.max(1, ...slotsOf(course).map((slot) => Math.max(1, Number(slot.end || slot.start || 1) - Number(slot.start || 1) + 1)), 1)
  const previewWidth = Math.max(116, Math.min(190, rect.width || 140))
  const previewHeight = Math.max(92, Math.min(190, (rect.height && rect.height > 34 ? rect.height : span * 58)))

  const clone = document.createElement('div')
  clone.className = `dragPreviewClone dragPreviewCourseCard ${variant === 'candidate' ? 'candidateDragPreview' : ''}`
  ...
  clone.innerHTML = `
    <b class="courseTypeDot ${STATUS[courseStatus(course)]?.tone || 'blue'}"></b>
  ...
  event.dataTransfer.setDragImage(clone, Math.min(previewWidth / 2, 82), Math.min(previewHeight / 2, 54))
...
function SemesterGrid({ semester, courses, plan, onDropCourse, onMoveCourse, onCourseClick, onEmptySlotClick, onDeleteCourse, onMoveToCandidate }) {
  ...
  function renderHalfSegment(course, extraClassName = '') {
    ...
    return (
      <button
        ...
        draggable
        onDragStart={(e) => { setCourseDragPreview(e, course); e.dataTransfer.setData('application/json', JSON.stringify({ source: 'planned', semester, course })) }}
  ...
  return (
    <section className="semesterPanel activeSemesterPanel" onDragOver={(e) => e.preventDefault()} onDrop={(e) => onDropCourse(e, semester)}>
                  ...
                  {hasStartingConflict ? (
                    ...
                    isHalfContinuation ? halfContinuationHitbox(activeCourses, di, p) : <button
                      ...
                      onDragStart={(e) => {
                        ...
                        setCourseDragPreview(e, continuationCourse)
      ...
      <div className="unscheduled">{courses.filter((course) => slotsOf(course).length === 0).map((course) => <button type="button" className="unscheduledItem glassCourse" key={uid(course)} data-drag-course-id={uid(course)} draggable onDragStart={(e) => { setCourseDragPreview(e, course); e.dataTransfer.setData('application/json', JSON.stringify({ source: 'planned', semester, course })) }} onClick={(e) => handleCourseClick(course, e)}><b className={`statusBadge ${STATUS[courseStatus(course)].tone}`} /><strong>{getCourse(course).name}</strong><small>{credits(course)} 學分｜無固定時間</small></button>)}</div>
