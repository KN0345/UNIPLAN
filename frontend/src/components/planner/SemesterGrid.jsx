...
function makeCourseDragPreview(event, course, options = {}) {
  const source = event.currentTarget
  if (!source || !event.dataTransfer || !course) return
  const detail = getCourse(course)
  const allSlots = slotsOf(course)
  const span = Math.max(1, ...allSlots.map((slot) => Math.max(1, Number(slot.end || slot.start || 1) - Number(slot.start || 1) + 1)), 1)
  const rect = source.getBoundingClientRect()
  const width = Math.max(118, Math.min(210, rect.width || 146))
  const height = Math.max(92, Math.min(210, (rect.height && rect.height > 40 ? rect.height : span * 58)))
  const meta = [scheduleRuleLabel(course), detail.classroom || detail.room || detail.location || '未列教室'].filter(Boolean).join('｜')
  const preview = document.createElement('div')
  preview.className = `dragPreviewCourseCard ${options.compact ? 'compactDragPreview' : ''}`
  ...
  dot.className = `courseTypeDot ${STATUS[courseStatus(course)]?.tone || 'blue'}`
  ...
  event.dataTransfer.setDragImage(preview, Math.min(width / 2, 92), Math.min(height / 2, 64))
