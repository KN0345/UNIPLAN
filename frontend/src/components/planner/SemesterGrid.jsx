...
function SemesterGrid({ semester, courses, plan, onDropCourse, onMoveCourse, onCourseClick, onEmptySlotClick, onDeleteCourse, onMoveToCandidate }) {
  ...
  function renderHalfSegment(course, extraClassName = '') {
    ...
    return (
      <button
        key={uid(course)}
        type="button"
        className={`halfSemesterSegment ${extraClassName} status-${STATUS[courseStatus(course)]?.tone || 'blue'}`}
  ...
  function tileButton(course, di, p, stackIndex = 0, stackCount = 1) {
    return (
      <button
        key={uid(course)}
        className={`timetableCourseTile ${stackCount > 1 ? 'stackedTile' : ''} status-${STATUS[courseStatus(course)]?.tone || 'blue'}`}
        data-drag-course-id={uid(course)}
        style={{
                ...
                <div className={`timetableCell ${hasCourse ? 'hasCourse' : ''}`} key={`${d}-${p}`}>
                  {hasStartingConflict ? (
                    ...
                    isHalfContinuation ? halfContinuationHitbox(activeCourses, di, p) : <button
                      type="button"
                      className={`occupiedContinuation ${hasContinuingConflict ? 'continuationConflict' : ''} status-${continuationCourse ? (STATUS[courseStatus(continuationCourse)]?.tone || 'blue') : 'blue'}`}
