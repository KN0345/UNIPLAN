...
function SemesterGrid({ semester, courses, plan, onDropCourse, onMoveCourse, onCourseClick, onEmptySlotClick, onDeleteCourse, onMoveToCandidate }) {
  ...
  function renderHalfSegment(course, extraClassName = '') {
    ...
    return (
      <button
        ...
        draggable
        onDragStart={(e) => { setCardDragPreview(e); e.dataTransfer.setData('application/json', JSON.stringify({ source: 'planned', semester, course })) }}
  ...
  return (
    <section className="semesterPanel activeSemesterPanel" onDragOver={(e) => e.preventDefault()} onDrop={(e) => onDropCourse(e, semester)}>
      ...
      <div className="unscheduled">{courses.filter((course) => slotsOf(course).length === 0).map((course) => <button type="button" className="unscheduledItem glassCourse" key={uid(course)} draggable onDragStart={(e) => { setCardDragPreview(e); e.dataTransfer.setData('application/json', JSON.stringify({ source: 'planned', semester, course })) }} onClick={(e) => handleCourseClick(course, e)}><b className={`statusBadge ${STATUS[courseStatus(course)].tone}`} /><strong>{getCourse(course).name}</strong><small>{credits(course)} 學分｜無固定時間</small></button>)}</div>
