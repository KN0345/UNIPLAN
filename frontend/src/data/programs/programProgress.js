...
function isInactiveCourse(course) {
  ...
  return status === 'failed' || status === 'withdrawn'
}

function programEligible(course) {
  const raw = course?.course || course || {}
  if (isInactiveCourse(raw)) return false
...
function getProgramCourseCode(programCourse) {
...
function getProgramCourseDepartment(programCourse) {
  return String(programCourse?.department || programCourse?.dept || programCourse?.unit || programCourse?.opening_unit || '').trim()
}

function getActualCourseCode(course) {
  const raw = course?.course || course || {}
  return String(raw.code || raw.course_code || raw.courseCode || raw.course_id || '').trim()
}

function getActualCourseDepartment(course) {
  const raw = course?.course || course || {}
  return String(raw.department || raw.dept || raw.major || raw.unit || raw.opening_unit || raw.category || '').trim()
}

function departmentCompatible(programCourse, course) {
  const expected = getProgramCourseDepartment(programCourse)
  if (!expected) return true
  const actual = getActualCourseDepartment(course)
...
export function matchCourse(programCourse, coursePool) {
  ...
  const expectedCode = getProgramCourseCode(programCourse)
  return coursePool.find(({ course }) => {
    const name = normBase(getCourseName(course))
    if (!name || !targets.has(name)) return false
    if (expectedCode) return getActualCourseCode(course) === expectedCode
    return departmentCompatible(programCourse, course)
  }) || null
}
