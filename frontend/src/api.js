...
export async function fetchCourses(params = {}) {
  try {
    ...
    if (data?.source === 'local-patches-fallback' && (data?.data || []).length < 1000) {
      const staticCourses = await loadStaticCourses()
      const filtered = staticCourses.filter((course) => matchesStaticCourse(course, params))
      return { ok: true, data: filtered, total: filtered.length, returned: filtered.length, source: 'static-courses-fallback', warning: data?.warning }
    }
    return data
  } catch (error) {
    const staticCourses = await loadStaticCourses()
...
export async function fetchMetadata() {
  try {
    ...
    if (data?.source === 'local-patches-fallback' && (data?.data?.departments || []).length < 100) {
      const staticCourses = await loadStaticCourses()
      return { ok: true, data: buildStaticMetadata(staticCourses), source: 'static-courses-fallback', warning: data?.warning }
    }
    return data
  } catch (error) {
    const staticCourses = await loadStaticCourses()
