...
import FlexibleProgramsPage from './pages/FlexibleProgramsPage'
...
function App() {
  ...
  const {
    ...
    accountProfile, setAccountProfile, handleLogin, handleResendVerification, handleGuestLogin, logout,
  } = useAccountState({
    notify,
    applyRemoteBundle,
    setPlan,
  ...
  useEffect(() => {
    function closeCourseInfoByEscape(event) {
      if (event.key === 'Escape') {
        setInfoOpen(false)
        setCoursePopover(null)
      }
    }
    window.addEventListener('keydown', closeCourseInfoByEscape, true)
    document.addEventListener('keydown', closeCourseInfoByEscape, true)
    return () => {
      window.removeEventListener('keydown', closeCourseInfoByEscape, true)
      document.removeEventListener('keydown', closeCourseInfoByEscape, true)
    }
  }, [setInfoOpen, setCoursePopover])
  const {
    candidateSearch,
    setCandidateSearch,
    candidateSort,
  ...
  return (
    <div className="appShell">
      ...
      <main className="main">
        ...
        {activeMenu === 'credits' && <CreditsPage profile={accountProfile} onProfileChange={setAccountProfile}><GraduationRulePreviewPanel profile={accountProfile} plan={plan} /></CreditsPage>}

        {activeMenu === 'flexiblePrograms' && <FlexibleProgramsPage profile={accountProfile} onProfileChange={setAccountProfile} plan={plan} courses={courses} candidates={candidates} onAddCandidate={addCandidate} onOpenCourseInfo={openCourseInfo} />}
        ...
        {infoOpen && <div className="infoOverlay" onMouseDown={(e) => { if (e.target === e.currentTarget) setInfoOpen(false) }}><div className="infoModalShell"><button className="closeInfo modalCloseFixed" onClick={() => setInfoOpen(false)}>關閉</button><div className="infoDrawerPanel"><CourseInfo course={selectedCourse} reviews={reviews} tagVotes={tagVotes} onTagVote={toggleTeacherTag} userId={userKey(user)} onAddReview={handleAddReview} onUpdateReview={updateLocalReview} onDeleteReview={deleteLocalReview} /></div></div></div>}
      </main>
