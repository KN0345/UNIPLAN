...
function courseSerialValue(course) {
  const c = getCourse(course)
  return String(
    c.serial
    || c.open_serial
...
function courseCodeValue(course) {
  const c = getCourse(course)
  return String(c.code || c.course_code || c.courseCode || c.course_id || c.subject_code || c['科目編號'] || c['課號'] || '').trim()
}

function courseSectionValue(course) {
  const c = getCourse(course)
  return String(c.class_name || c.className || c.class || c.section || c.major || c['班別'] || c['班級'] || '').trim()
}

function courseIdentityFallback(course) {
  const c = getCourse(course)
  return [
    courseCodeValue(c),
    courseSectionValue(c),
    c.department || c.unit || c.major || '',
    c.name || c.course_name || '',
    c.teacher || c.instructor || '',
    c.time_info || c.time || c.time_data || '',
    c.classroom || c.room || c.location || '',
  ]
...
export function uid(course) {
  const c = getCourse(course)
  const term = courseCatalogTermValue(c) || String(c.semester_source || c.semester || '')
  const serial = courseSerialValue(c)
  const key = serial ? `${term}:serial:${serial}` : `${term}:fallback:${courseIdentityFallback(c)}`
  return String(c.uid || c.instanceId || c.id || key || `${c.name || 'course'}-${Math.random()}`)
}
...
export function courseKey(course) {
  ...
  const serial = courseSerialValue(c)
  if (serial) return term ? `${term}:serial:${serial}` : `serial:${serial}`

  const fallback = courseIdentityFallback(c)
  if (fallback) return term ? `${term}:fallback:${fallback}` : `fallback:${fallback}`

  const id = String(c.id || '').trim()
  return term ? `${term}:id:${id || 'unknown'}` : `id:${id || 'unknown'}`
}
...
function buildStableExportDom(element, semester = '課表') {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    cells.forEach((cell, dayIndex) => {
      ...
      const spanRaw = tile?.dataset?.tileSpan || tile?.style?.getPropertyValue('--tile-span') || '1'
  ...
  rows.slice(0, periodCount).forEach((row, rowIndex) => {
    ...
    keepDays.forEach((dayIndex, columnIndex) => {
      ...
      tiles.forEach((tile, tileIndex) => {
        const spanRaw = tile.dataset?.tileSpan || tile.style.getPropertyValue('--tile-span') || window.getComputedStyle(tile).getPropertyValue('--tile-span') || '1'
...
function prepareCompactTimetableExport(root, fallbackWidth = 1180, fallbackHeight = 780) {
  ...
  grid.querySelectorAll('.timetableCourseTile,.timetableConflictTile').forEach((tile) => {
    const rawSpan = tile.dataset?.tileSpan
      || tile.style.getPropertyValue('--tile-span')
