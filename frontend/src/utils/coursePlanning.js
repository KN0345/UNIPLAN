...
export const COURSE_CATALOG_TERMS = [
  ...
  { value: '1151CLASS', label: '115 學年度上學期' },
]
...
export const STATUS = {
  ...
  withdrawn: { label: '停修', tone: 'purple' },
}
export const STATUS_ORDER = ['planned', 'completed', 'failed', 'withdrawn']
...
export function candidateGroupKey(course) {
  const c = getCourse(course)
  const term = String(courseCatalogTermValue(c) || c.semester_source || c.semester || c.term || '').trim()
  const code = String(c.code || c.course_code || c.courseCode || c.course_id || '').trim()
  const rawName = String(c.name || c.course_name || '').trim()
  const name = normalizedCourseNameForMerge(c)
  const dept = String(c.department || c.department_code || c.unit || c.opening_unit || '').trim()
  const credit = String(c.credits ?? c.credit ?? '').trim()

  // Same course, different classes/teachers/times should be treated as one candidate group.
  // Normalize class suffixes and spaces so 法文（一）A/B or same-name sections are grouped.
  // Keep term in the group key so 114上 / 114下 versions are not mixed.
  if (code || name) return [term, code, name || rawName, credit].filter(Boolean).join('::')
  return [term, dept, rawName, credit].filter(Boolean).join('::') || courseKey(course)
}
...
export function isCourseAlreadyPlanned(plan, course) {
  ...
  return Object.values(plan)
    ...
    .flat()
    .some((item) => !isInactiveCourse(item) && courseKey(item) === key)
}
...
export function isInactiveCourse(course) {
  // failed / withdrawn are retained as history records only.
  // They do not count toward credits, conflicts, graduation progress, or duplicate blocking.
  return ['failed', 'withdrawn'].includes(courseStatus(course))
}

export function courseStatus(course) {
  const c = getCourse(course)
  return c?.planningStatus || course?.planningStatus || c?.status || c?.planning_status || course?.status || course?.planning_status || 'planned'
}
...
export function findConflict(course, courses) {
  return courses.filter((c) => !isInactiveCourse(c)).find((c) => hasConflict(course, c))
}
...
export function effectiveCourses(plan) {
  ...
  return Object.values(plan)
    .filter(Array.isArray)
    .flat()
    .filter((c) => c && !isInactiveCourse(c))
    .filter((c) => !(getCourse(c).manual && getCourse(c).includeGraduation === false))
}


function courseTextForAudit(course) {
  const c = getCourse(course)
  return `${c.code || ''} ${c.name || ''} ${c.department || ''} ${c.group_type || ''} ${c.category || ''} ${c.notes || ''}`
}

function normalizedAuditCourseKey(course) {
  const c = getCourse(course)
  const code = String(c.code || '').trim()
  const name = normalizedCourseNameForMerge(course).replace(/[-－–—]\s*(入門課程|進階課程)$/u, '')
  const term = courseCatalogTermValue(c)
  return [term, code, name].filter(Boolean).join(':') || courseMergeIdentity(course)
}

export function universityRequirementType(course) {
  const c = getCourse(course)
  const name = String(c.name || '')
  const dept = String(c.department || '')
  const code = String(c.code || '')
  const group = String(c.group_type || '')
  const text = courseTextForAudit(c)

  if (/校園與社區服務學習/.test(name)) return 'serviceLearning'
  if (/全民國防教育軍事訓練.*國防科技|國防科技/.test(name)) return 'nationalDefense'
  if (/體育/.test(name) && /T9869|體育/.test(text)) return 'physicalEducation'
  if (/人工智慧導論/.test(name)) return 'aiIntro'
  if (/探索永續/.test(name)) return 'sustainability'
  if (/大學學習/.test(name)) return 'universityLearning'
  if (/學習與發展/.test(name) || (group === 'N' && /核心課程Ｎ群/.test(dept))) return 'learningDevelopment'
  if (/社團學習與實作/.test(name) || code === 'T2637') return 'clubPractice'
  if (/中國語文能力表達/.test(name)) return 'chineseExpression'
  if ((/^英文[（(一二]/.test(name) || /^英文\s*[（(]/.test(name)) && (/英文/.test(dept) || /^A\d+/.test(code))) return 'foreignLanguage'

  if (/核心課程[ＯＵＺ]群/.test(dept) || ['O', 'U', 'Z'].includes(group)) return 'generalScience'
  if (/核心課程[ＲＳＴＷ]群/.test(dept) || ['R', 'S', 'T', 'W'].includes(group)) return 'generalSocial'
  if (/核心課程[ＬＰＶＭ]群/.test(dept) || ['L', 'P', 'V', 'M'].includes(group)) return 'generalHumanities'
  return ''
}

export function graduationCountedCredits(course) {
  const type = universityRequirementType(course)
  if (type === 'nationalDefense' || type === 'physicalEducation') return 0
  return credits(course)
}

export const UNIVERSITY_REQUIREMENT_RULE_114 = {
  service: [
    { key: 'serviceLearning', label: '校園與社區服務學習', target: 2, counted: true },
    { key: 'nationalDefense', label: '國防科技', target: 1, counted: false },
    { key: 'physicalEducation', label: '體育', target: 4, counted: false },
  ],
  basic: [
    { key: 'clubPractice', label: '社團學習與實作', target: 1, counted: true },
    { key: 'universityLearning', label: '大學學習', target: 1, counted: true },
    { key: 'foreignLanguage', label: '外國語文', target: 8, counted: true },
    { key: 'chineseExpression', label: '中國語文能力表達', target: 2, counted: true },
  ],
  core: [
    { key: 'aiIntro', label: '人工智慧導論', target: 1, counted: true },
    { key: 'sustainability', label: '探索永續', target: 1, counted: true },
    { key: 'generalScience', label: '科學領域', target: 2, counted: true },
    { key: 'generalSocial', label: '社會領域', target: 2, counted: true },
    { key: 'generalHumanities', label: '人文領域', target: 2, counted: true },
  ],
}

function cappedRequirementValue(items, key, target = Infinity) {
  const value = Number(items?.[key]?.credits || 0)
  return Math.min(value, target)
}

export function universityRequirementAudit(plan) {
  ...
  courses.forEach((course) => {
    const type = universityRequirementType(course)
    if (!type) return
    const key = `${type}:${normalizedAuditCourseKey(course)}`
    if (seen.has(key)) return
    seen.add(key)
    if (!items[type]) items[type] = { credits: 0, courses: [] }
    items[type].credits += credits(course)
    items[type].courses.push(course)
  })

  const serviceCounted = cappedRequirementValue(items, 'serviceLearning', 2)
  const basicCounted = cappedRequirementValue(items, 'clubPractice', 1)
    + cappedRequirementValue(items, 'universityLearning', 1)
    + cappedRequirementValue(items, 'learningDevelopment', 1)
    + cappedRequirementValue(items, 'foreignLanguage', 8)
    + cappedRequirementValue(items, 'chineseExpression', 2)
  const coreCounted = cappedRequirementValue(items, 'aiIntro', 1)
    + cappedRequirementValue(items, 'sustainability', 1)
    + cappedRequirementValue(items, 'generalScience', Infinity)
    + cappedRequirementValue(items, 'generalSocial', Infinity)
    + cappedRequirementValue(items, 'generalHumanities', Infinity)
  ...
  return {
    ...
    basicCounted,
    coreCounted,
    countedUniversityCredits: serviceCounted + Math.min(basicCounted, 12) + Math.min(coreCounted, 12),
    targets: { service: 2, basic: 12, core: 12, university: 26 },
  }
}



function isHonorProgramCourse(course) {
  const c = getCourse(course)
  const text = normalizeDepartmentForCredit(`${c.department || ''} ${c.major || ''} ${c.unit || ''} ${c.opening_unit || ''} ${c.name || ''}`)
  const code = String(c.code || '').trim().toUpperCase()
  return /榮譽學程|honorsprogram|tgahb|tgshb|tgehb|tglhb|tgfhb|tgrhb|tgkhb|tgdhb/.test(text) || /^TG[A-Z]HB$/.test(code)
}

function isCollegeCommonCourse(course) {
  const c = getCourse(course)
  const text = normalizeDepartmentForCredit(`${c.department || ''} ${c.major || ''} ${c.unit || ''} ${c.opening_unit || ''} ${c.notes || ''} ${c.name || ''}`)
  const code = String(c.code || '').trim().toUpperCase()
  const departmentRaw = String(c.department || c.major || c.unit || c.opening_unit || '').toUpperCase()

  // 所有學院共同科 = 院必修。
  // 涵蓋：文、理、工、商管、外語、國際、教育、AI創智等共同科，
  // 以及共同科日/碩/博/EMBA等不同層級。
  if (/共同科目|共同科|學院共同|院共同|教育共同科|教育學院共同|college|electivescoursesbycollege/.test(text)) return true

  // 淡江共同科常見代碼：
  // TGAXB 文學院、TGSXB 理學院、TGEXB 工學院、TGLXB 商管、
  // TGFXB 外語、TGRXB 國際、TGDXB 教育、TGKXB AI創智。
  // 後綴 B/M/D/J 等可能代表日間、碩士、博士、EMBA或其他共同科分類。
  if (/TG[A-Z]{2}[BMDJX]/.test(departmentRaw)) return true
  if (/^TG[A-Z]{2}[BMDJX]$/.test(code)) return true

  // 已知特殊：遠距教學課程、教育學院共同碩等，也歸院必修。
  if (/TGDXB|TGDXM|TGDLB|TGEXM|TGLXM|TGSXM|TGFXM|TGRXM|TGKXM/.test(departmentRaw)) return true
  if (/遠距教學課程|domesticlongdistancecourse/.test(text)) return true

  return false
}

function isCenterHolisticCourse(course) {
  const c = getCourse(course)
  const text = normalizeDepartmentForCredit(`${c.department || ''} ${c.name || ''} ${c.notes || ''}`)
  const code = String(c.code || '').trim().toUpperCase()
  return /^TGPNB$/.test(code) || /全教育共同科目|centerforholisticeducation|社會議題探索暨實踐|團隊發展|國際探索/.test(text)
}

function normalizeDepartmentForCredit(value = '') {
  return String(value || '').replace(/[()\s（）／/:：\-—_]/g, '').toLowerCase()
}

function isEducationTechDepartment(course) {
  const c = getCourse(course)
  const dept = normalizeDepartmentForCredit(`${c.department || ''} ${c.major || ''} ${c.unit || ''} ${c.opening_unit || ''}`)
  return /教育科技|教科|教育科技學系|tdtx|tdux|tetc|教育與科技|教育未來/.test(dept)
}

function isClearlyOtherDepartment(course) {
  const c = getCourse(course)
  const dept = normalizeDepartmentForCredit(`${c.department || ''} ${c.major || ''} ${c.unit || ''} ${c.opening_unit || ''}`)
  if (!dept) return false
  if (isEducationTechDepartment(course)) return false
  return /企管|企業管理|觀光|資管|資訊管理|會計|財金|經濟|中文|英文|日文|法文|西文|俄文|外交|歷史|大傳|資傳|統計|數學|物理|化學|電機|機械|土木|水環|航太|建築/.test(dept)
}

function isUniversityLevelCredit(course) {
  const c = getCourse(course)
  const text = `${c.category || ''} ${c.group_type || ''} ${c.department || ''} ${c.name || ''} ${c.required_type || ''} ${c.requiredType || ''} ${c.notes || ''}`
  return /校必|通識|共同|英文|外國語文|中國語文|探索永續|人工智慧導論|體育|國防|服務學習|核心課程/.test(text)
}
...
export function creditCategory(course) {
  ...
  // 補丁規則：院共同科目就是院必修；全教育共同科目歸校必修。
  if (isHonorProgramCourse(c)) return 'freeElective'
  if (isCenterHolisticCourse(c)) return 'universityRequired'
  if (isCollegeCommonCourse(c)) return 'collegeRequired'

  if (isUniversityLevelCredit(c)) return 'universityRequired'
  if (/院必|院級/.test(text)) return 'collegeRequired'

  // Prevent other departments' required courses from being counted as the user's 系必修.
  if (isClearlyOtherDepartment(c)) return 'freeElective'

  if (isEducationTechDepartment(c) && (/系必|系訂必修|專業必修/.test(text) || req === '必修')) return 'departmentRequired'
  if (isEducationTechDepartment(c) && (/系選|專業選修/.test(text) || req === '選修')) return 'departmentElective'
  if (/系必|系訂必修|專業必修/.test(text)) return 'departmentRequired'
  if (/系選|專業選修/.test(text)) return 'departmentElective'
  return 'freeElective'
}

export function creditSummary(plan, rules = DEFAULT_RULES) {
  const courses = effectiveCourses(plan)
  const total = courses.reduce((sum, c) => sum + graduationCountedCredits(c), 0)
  const completed = courses.filter((c) => courseStatus(c) === 'completed').reduce((sum, c) => sum + graduationCountedCredits(c), 0)
  const planned = courses.filter((c) => courseStatus(c) !== 'completed').reduce((sum, c) => sum + graduationCountedCredits(c), 0)
  ...
  courses.forEach((course) => {
    ...
    byCat[key] = (byCat[key] || 0) + graduationCountedCredits(course)
    if (courseStatus(course) === 'completed') byCatCompleted[key] = (byCatCompleted[key] || 0) + graduationCountedCredits(course)
  })
...
export function semesterWarnings(semester, courses, plan) {
  const active = courses.filter((c) => !isInactiveCourse(c))
  const warnings = []
  ...
  const failed = []
  // 停用被當提示：被當與停修仍保留歷程，但不再顯示橘色警示。
  return [...new Set(warnings)]
}

export function occupied(courses) {
  const set = new Set()
  courses.filter((c) => !isInactiveCourse(c)).forEach((course) => slotsOf(course).forEach((slot) => {
...
export function semesterCreditStatus(courses) {
  const total = courses.filter((c) => !isInactiveCourse(c)).reduce((sum, c) => sum + credits(c), 0)
...
export async function exportCleanPng(plan, semester = '課表') {
  ...
  const activeCourses = rawCourses.filter((course) => !isInactiveCourse(course))
  const allSlots = []
...
export function exportCalendar(plan, semester = '') {
  ...
  entries.forEach(([sem, courses]) => courses.filter((c) => !isInactiveCourse(c)).forEach((course, i) => {
    const c = getCourse(course)
