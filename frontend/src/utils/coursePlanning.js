...
function isHonorProgramCourse(course) {
  ...
  return /榮譽學程|honorsprogram|tga hb|tgahb|tgshb|tgehb|tglhb|tgfhb|tgrhb|tgkhb|tgdhb/.test(text) || /^TG[A-Z]HB$/.test(code)
}

function isCollegeCommonCourse(course) {
  const c = getCourse(course)
  const text = normalizeDepartmentForCredit(`${c.department || ''} ${c.major || ''} ${c.unit || ''} ${c.opening_unit || ''} ${c.notes || ''}`)
  const code = String(c.code || '').trim().toUpperCase()
  if (/共同科目|college|electivescoursesbycollege/.test(text)) return true
  return /^TG[A-Z]XB$/.test(code) || /^TG[A-Z]XM$/.test(code)
}
