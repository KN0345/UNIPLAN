...
export function stableCourseIdentity(course) {
  const c = getCourse(course)
  const term = courseCatalogTermValue(c) || String(c.semester_source || c.semester || c.term || c.source_term || c.catalog_term || '').trim()
  const serial = String(
    c.serial
    || c.open_serial
    || c.opening_serial
  ...
  // 淡江大量共同課（T0466/A0050 英文）會在不同學院重複 A/B/C 班。
  // 唯一識別必須優先使用「學期 + 開課序號」，避免被 科目編號 + 班別 吃掉。
  if (serial) return `${term || 'term'}:serial:${serial}`

  const dept = String(c.department || c.department_code || c.dept || c.unit || '').trim()
  const code = String(c.code || c.course_code || c.course_id || c.courseCode || '').trim()
  ...
  return [term || 'term', dept, code || name, cls, teacher, time].filter(Boolean).join(':')
}

export function uid(course) {
  const c = getCourse(course)
  return String(c.uid || c.instanceId || stableCourseIdentity(c) || `${c.name || 'course'}-${Math.random()}`)
}
