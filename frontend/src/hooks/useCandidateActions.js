...
import { courseKey, credits, getCourse, isCourseAlreadyPlanned, isInactiveCourse } from '../utils/coursePlanning'
...
export function useCandidateActions({ plan, candidates, setCandidates, favorites, setFavorites, notify }) {
  ...
  function putCandidate(course, { allowPlanned = false, silent = false } = {}) {
    ...
    // 被當或停修只是歷程紀錄，不代表這門課已被有效安排。
    // 因此暫存區只用「有效狀態」課程做去重；若既有項目是 failed/withdrawn，仍允許再次加入正常排課版本。
    const exists = candidates.some((c) => !isInactiveCourse(c) && courseKey(c) === courseKey(item))
    if (exists) {
