...
export function usePlannerActions({
  ...
  function autoPlace(target) {
    ...
    if (!semesters.length) {
      notify('請先選擇智慧排入的學期範圍')
      return { placed: 0, skipped: candidates.length, bySemester: {}, groups: 0, alternatives: 0 }
    }
    ...
    candidates.forEach((course, index) => {
      const key = candidateGroupKey(course)
      if (!grouped.has(key)) grouped.set(key, [])
      grouped.get(key).push({ course, index })
    })

    const groups = Array.from(grouped.entries()).map(([groupKey, entries]) => {
      const sample = entries[0]?.course
      const matchingCount = entries.filter(({ course }) => semesters.some((semester) => courseMatchesSemester(course, semester))).length
      const maxSlotCount = Math.max(0, ...entries.map(({ course }) => slotsOf(course).length))
      return {
        groupKey,
        entries,
        matchingCount,
      ...
      const matchDelta = (a.matchingCount || 999) - (b.matchingCount || 999)
      if (matchDelta) return matchDelta
      const slotDelta = b.maxSlotCount - a.maxSlotCount
    ...
    function placementOptionsFor(course) {
      ...
      return matchingSemesters.map((semester) => {
        ...
        const courseSlots = slotsOf(course)
        const dayCount = new Set(courseSlots.map((slot) => slot.day)).size
        const earlyPenalty = courseSlots.some((slot) => slot.start <= 1) ? 0.8 : 0
        const latePenalty = courseSlots.some((slot) => slot.end >= 9) ? 0.35 : 0
        const saturdayPenalty = courseSlots.some((slot) => slot.day === 6) ? 0.45 : 0
        const sundayPenalty = courseSlots.some((slot) => slot.day === 7) ? 1.2 : 0
        const fragmentPenalty = dayCount * 0.18
        const creditBalancePenalty = currentCredits * 0.08
        return {
          semester,
          rejected: false,
          score: (afterPenalty - beforePenalty) + earlyPenalty + latePenalty + saturdayPenalty + sundayPenalty + fragmentPenalty + creditBalancePenalty,
        }
      })
    }

    groups.forEach((group) => {
      alternatives += Math.max(0, group.entries.length - 1)
      const allOptions = []
      let sawDuplicate = false
      let sawWrongTerm = false
      let sawConflict = false
      let sawCredit = false

      group.entries.forEach(({ course, index }) => {
        const options = placementOptionsFor(course)
        options.forEach((option) => {
          if (option.duplicate) sawDuplicate = true
          if (option.wrongTerm) sawWrongTerm = true
          if (option.conflict) sawConflict = true
          if (option.overCredit) sawCredit = true
          if (!option.rejected) {
            allOptions.push({
              ...option,
              course,
              index,
              optionScore: option.score + index * 0.001,
            })
          }
        })
      })

      allOptions.sort((a, b) => a.optionScore - b.optionScore)
      const best = allOptions[0]
      if (!best) {
        if (sawDuplicate) skippedDuplicate += group.entries.length
        else if (sawWrongTerm) skippedWrongTerm += group.entries.length
        else if (sawCredit && !sawConflict) skippedCredit += group.entries.length
        else skippedConflict += group.entries.length
        return
      }

      const item = setCourseStatus(best.course, 'planned')
      next[best.semester].push({ ...item, instanceId: `${courseKey(item)}-${Date.now()}-${placed}` })
      placedKeys.add(courseKey(best.course))
      placedGroupKeys.add(group.groupKey)
      bySemester[best.semester] = (bySemester[best.semester] || 0) + 1
      placed += 1
    })

    if (placed) {
      setCandidates((prev) => prev.filter((course) => {
        const key = courseKey(course)
        const groupKey = candidateGroupKey(course)
        return !placedKeys.has(key) && !placedGroupKeys.has(groupKey)
      }))
    }

    commit(next, rangeMode ? `智慧排入 ${semesters[0]}～${semesters.at(-1)}：${placed} 門` : `智慧排入 ${semesters[0]}：${placed} 門`)
    const skipped = candidates.length - placed
    const summary = {
      placed,
      skipped,
      skippedWrongTerm,
    ...
    notify(placed ? `智慧排入完成：${placed} 門，已自動從 ${groups.length} 組候選班別中擇優；${detail}` : `沒有可排入課程；未排入 ${skipped} 門`)
    return summary
  }
  ...
  function clearTimetable(scope = 'active', options = {}) {
    const targetSemester = scope === 'all' ? null : activeSemester
    const hasAnyCourse = scope === 'all'
      ? Object.values(plan || {}).some((courses) => Array.isArray(courses) && courses.length)
      : Array.isArray(plan?.[targetSemester]) && plan[targetSemester].length
    if (!hasAnyCourse) {
      notify(scope === 'all' ? '全部課表目前是空的' : `${targetSemester} 課表目前是空的`)
      return false
    }
    if (options.confirmed !== true) return false
    const next = { ...plan }
    if (scope === 'all') {
      SEMESTERS.forEach((semester) => { next[semester] = [] })
      commit(next, '清除全部課表')
      notify('已清除全部學期課表')
      return true
    }
    next[targetSemester] = []
    commit(next, `清除 ${targetSemester} 課表`)
    notify(`已清除 ${targetSemester} 課表`)
    return true
  }
