...
export function usePlannerActions({
  ...
  function clearTimetable(scope = 'all') {
    const hasAnyCourse = Object.values(plan || {}).some((courses) => Array.isArray(courses) && courses.length)
    if (!hasAnyCourse) {
      notify('課表目前是空的')
      return false
    }
    const message = scope === 'active'
      ? `確定要清除 ${activeSemester} 的課表嗎？此操作可用復原還原。`
      : '確定要清除全部學期課表嗎？此操作可用復原還原。'
    if (!window.confirm(message)) return false
    const next = { ...plan }
    if (scope === 'active') {
      next[activeSemester] = []
      commit(next, `清除 ${activeSemester} 課表`)
      notify(`已清除 ${activeSemester} 課表`)
      return true
    }
