...
export function usePlannerActions({
  ...
  function clearTimetable(options = {}) {
    const targetSemester = activeSemester
    const hasAnyCourse = Array.isArray(plan?.[targetSemester]) && plan[targetSemester].length
    if (!hasAnyCourse) {
      notify(`${targetSemester} 課表目前是空的`)
      return false
    }
    if (options.confirmed !== true) return false
    const next = { ...plan, [targetSemester]: [] }
