...
export function useCourseSearchState({ activeSemester, favorites = [], candidates = [], plan = [], tagVotes = {} }) {
  ...
  const sortedFilteredCourses = useMemo(() => {
    ...
    const deptTarget = searchFilters.department && searchFilters.department !== '全部'
      ? normalizeSearchText(searchFilters.department)
      : ''
    const parsedDeptTarget = parsedQuery.department ? normalizeSearchText(parsedQuery.department) : ''
    ...
    const entries = indexedCourses.filter((entry) => {
      ...
      if (deptTarget && !(entry.departmentText.includes(deptTarget) || deptTarget.includes(entry.departmentText))) return false
      ...
      if (parsedDeptTarget && !entry.departmentText.includes(parsedDeptTarget)) return false
