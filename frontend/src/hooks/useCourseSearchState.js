...
const COURSE_RESULT_RENDER_LIMIT = 320
...
export function useCourseSearchState({ activeSemester, favorites = [], candidates = [], plan = [], tagVotes = {} }) {
  ...
  const sortedFilteredCourses = useMemo(() => {
    ...
    const list = (courses || []).filter((course) => {
      ...
      if (!parsedQuery.department && searchFilters.department && searchFilters.department !== '全部') {
        ...
        const deptMatched = extractDepartmentCandidates(course).some((item) => {
          const deptText = normalizeSearchText(item)
          return deptText.includes(deptTarget) || deptTarget.includes(deptText)
        })
      ...
      if (deferredQuery && !courseMatchesParsed(course, parsedQuery)) return false
      return true
    })
    ...
    if (tagFilter !== '全部') {
      ...
      }).slice(0, visibleCourseLimit(deferredQuery))
    }
    const queryText = String(deferredQuery || '').trim()
    if (queryText && (searchSort === 'smart' || searchSort === 'default')) {
      return list.sort((a, b) => {
        const diff = relevanceScoreParsed(b, parsedQuery) - relevanceScoreParsed(a, parsedQuery)
        if (diff) return diff
    ...
    if (searchSort === 'credits' || searchSort === 'creditsDesc') return list.sort((a, b) => credits(b) - credits(a)).slice(0, visibleCourseLimit(deferredQuery))
    if (searchSort === 'name') return list.sort((a, b) => String(getCourse(a).name || '').localeCompare(String(getCourse(b).name || ''), 'zh-Hant')).slice(0, visibleCourseLimit(deferredQuery))
    if (searchSort === 'favorite') return list.sort((a, b) => Number(favorites.some((f) => courseKey(f) === courseKey(b))) - Number(favorites.some((f) => courseKey(f) === courseKey(a)))).slice(0, visibleCourseLimit(deferredQuery))
    if (searchSort === 'candidate') return list.sort((a, b) => Number(candidates.some((c) => courseKey(c) === courseKey(b))) - Number(candidates.some((c) => courseKey(c) === courseKey(a)))).slice(0, visibleCourseLimit(deferredQuery))
    return list.sort((a, b) => {
  ...
  }, [courses, activeSemester, searchOnlyAvailable, searchSort, favorites, candidates, plan, searchFilters.tag, searchFilters.department, searchFilters.requirement, searchFilters.grade, tagVotes, deferredQuery, metadata, departmentPool])
