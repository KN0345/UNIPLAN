...
export function useCourseSearchState({ activeSemester, favorites = [], candidates = [], plan = [], tagVotes = {} }) {
  ...
  const sortedFilteredCourses = useMemo(() => {
    ...
    const departmentPool = Array.from(new Set([...(metadata.departments || []), ...(courses || []).map((course) => getCourse(course).department).filter(Boolean)]))
    ...
    const list = (courses || []).filter((course) => {
      ...
      if (searchFilters.department && searchFilters.department !== '全部' && !String(c.department || '').includes(searchFilters.department)) return false
      ...
      if (parsedQuery.department && !normalizeSearchText(c.department || '').includes(normalizeSearchText(parsedQuery.department))) return false
