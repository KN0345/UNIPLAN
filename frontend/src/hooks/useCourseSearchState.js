...
const COURSE_RESULT_RENDER_LIMIT = 480
const COURSE_RESULT_QUERY_LIMIT = 620
...
export function useCourseSearchState({ activeSemester, favorites = [], candidates = [], plan = [], tagVotes = {} }) {
  ...
  const sortedFilteredCourses = useMemo(() => {
    ...
    if (searchSort === 'credits' || searchSort === 'creditsDesc') return limitedCourses(list.sort((a, b) => credits(b.course) - credits(a.course)))
    if (searchSort === 'name') return limitedCourses(list.sort((a, b) => a.nameSort.localeCompare(b.nameSort, 'zh-Hant')))
    if (searchSort === 'favorite') return limitedCourses(list.sort((a, b) => Number(favoriteKeys.has(b.courseKey)) - Number(favoriteKeys.has(a.courseKey))))
    if (searchSort === 'candidate') return limitedCourses(list.sort((a, b) => Number(candidateKeys.has(b.courseKey)) - Number(candidateKeys.has(a.courseKey))))
