...
function departmentAliases(value = '') {
  ...
  if (/觀光|觀光事業/.test(text)) ['觀光', '觀光系', '觀光事業', '觀光事業系', '觀光事業學系'].forEach((x) => aliases.add(x))
  if (/大傳|大眾傳播/.test(text)) ['大傳', '大傳系', '大眾傳播', '大眾傳播系', '大眾傳播學系'].forEach((x) => aliases.add(x))
  if (/英文/.test(text)) ['英文', '英文系', '英文學系'].forEach((x) => aliases.add(x))
  if (/歷史/.test(text)) ['歷史', '歷史系', '歷史學系'].forEach((x) => aliases.add(x))
  if (/國企|國際企業/.test(text)) ['國企', '國企系', '國際企業', '國際企業系', '國際企業學系'].forEach((x) => aliases.add(x))
  if (/財金|財務金融/.test(text)) ['財金', '財金系', '財務金融', '財務金融系', '財務金融學系'].forEach((x) => aliases.add(x))
  if (/會計/.test(text)) ['會計', '會計系', '會計學系'].forEach((x) => aliases.add(x))
...
function fieldMatchScore(text, q, exact, starts, includes) {
  if (!q) return 0
  if (text === q) return exact
  if (text.startsWith(q)) return starts
  if (text.includes(q)) return includes
  return 0
}
...
function entryRelevanceScore(entry, parsedQuery) {
  ...
  // 搜尋排序固定遵守：完全匹配 > 開頭匹配 > 包含匹配。
  // 欄位權重仍保留：課號 / 課名 > 教師 > 開課單位。
  score += fieldMatchScore(entry.code, q, 1400, 900, 420)
  score += fieldMatchScore(entry.baseName, q, 1300, 820, 520)
  score += fieldMatchScore(entry.name, q, 1200, 760, 480)
  score += fieldMatchScore(entry.teacher, q, 700, 360, 160)
  score += fieldMatchScore(entry.unit, q, 420, 210, 90)

  if (deptQuery && !entry.departmentText.includes(deptQuery)) score -= 260
  if (q && score > 0) score += Math.max(0, 80 - Math.abs(entry.name.length - q.length) * 4)
  return score
}
