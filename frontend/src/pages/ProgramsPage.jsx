...
import { courseStatus, isInactiveCourse, extractCourseList } from '../utils/coursePlanning'
...
function isPlannedProgramMatch(programCourse, pool) {
  return Boolean(matchCourse(programCourse, (pool || []).filter(({ course }) => courseStatus(course) !== 'completed' && !isInactiveCourse(course))))
}
...
function offeringMatchesKeyword(course, keyword) {
  const c = getCourse(course)
  return fuzzyIncludesSmart(`${c.name || ''} ${c.serial || ''} ${c.code || ''} ${cleanTeacherName(c.teacher || c.instructor)} ${courseTimeRoom(c)} ${c.classroom || c.room || c.location || ''}`, keyword)
}
...
export default function ProgramsPage({ profile, plan, candidates, favorites, courses = [], onAddCandidate, onOpenCourseInfo }) {
  ...
  const pool = useMemo(() => [
    ...Object.values(plan || {}).flat().filter((course) => !isInactiveCourse(course)).map((course) => ({ course, source: '已排' })),
  ...
  function toggleCourse(group, programCourse) {
    ...
    openCoursePicker(programCourse, group, false)
  }
  ...
  async function openCoursePicker(programCourse, group = null, autoSelectSingle = false) {
    ...
    setCoursePickerQuery(''); setCoursePicker({ group, programCourse, matches: localMatches, loading: true, source: localMatches.length ? '目前清單' : '搜尋中' })
    try {
  ...
  return <section className="pageCard programPage programChooserPage">
          ...
          return <section className="programChoiceGroup" key={group.id}>
            ...
            <div className="programCourseList">{(group.courses || []).map((programCourse) => {
              ...
              return <article key={`${group.id}-${programCourse.name}`} className={`programCourseRow ${state}`}>
                ...
                <button type="button" className="programCourseInfoDot" aria-label="選擇或查看開課班別" onClick={(event) => { event.stopPropagation(); openCoursePicker(programCourse, group, false) }}>i</button>
              </article>
            })}</div>
          </section>
        })}
    ...
    {coursePicker && createPortal(<div className="flexOfferingModalBackdrop" onMouseDown={() => setCoursePicker(null)}>
      <section className="flexOfferingModal" onMouseDown={(event) => event.stopPropagation()}>
        <header><div><span>{coursePicker.source || '選擇開課班別'}</span><h3>{coursePicker.programCourse.name}</h3></div><button type="button" onClick={() => setCoursePicker(null)} aria-label="關閉">×</button></header>
        <div className="flexOfferingSearch">
          <input value={coursePickerQuery} onChange={(event) => setCoursePickerQuery(event.target.value)} placeholder="搜尋序號、教師、時間、教室" autoFocus />
          <small>{(coursePicker.matches || []).filter((match) => offeringMatchesKeyword(match, coursePickerQuery)).length}/{coursePicker.matches?.length || 0} 筆</small>
        </div>
        <div className="flexOfferingList">
          {coursePicker.loading ? <div className="emptyVariantBox"><b>搜尋中</b></div> : coursePicker.matches?.length ? coursePicker.matches.filter((match) => offeringMatchesKeyword(match, coursePickerQuery)).map((match) => {
            const mc = getCourse(match)
            const already = isAlreadyInPool(mc, pool)
            return <article key={dedupeCourseKey(mc)} className="flexOfferingRow">
              <div>
                <strong>{mc.serial || mc.code || '無序號'}</strong>
                <span>{cleanTeacherName(mc.teacher) || '未列教師'}</span>
                <small>{courseTimeRoom(mc)}｜{mc.credits || mc.credit || coursePicker.programCourse.credits || 0}學分</small>
              </div>
              <div>
                <button type="button" onClick={() => onOpenCourseInfo?.(mc)}>資訊</button>
                <button type="button" disabled={already} onClick={() => selectVariantFromPicker(mc)}>{already ? '已在規劃中' : '選擇'}</button>
              </div>
            </article>
