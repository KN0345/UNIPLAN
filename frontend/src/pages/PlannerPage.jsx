...
export default function PlannerPage(props) {
  ...
  return (
    <>
      ...
      {showClearTimetableConfirm && <div className="manualCourseBackdrop plannerModalOverlay" onMouseDown={() => setShowClearTimetableConfirm(false)}>
        <section className="manualCourseModal autoPlanModal compactConfirmModal" onMouseDown={(event) => event.stopPropagation()}>
          <header><div><h3>清除當前學期課表</h3><p>只會清除「{activeSemester}」課表，不會影響其他學期。清除後仍可用復原還原。</p></div><button onClick={() => setShowClearTimetableConfirm(false)}>×</button></header>
