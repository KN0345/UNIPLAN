...
function cleanupCandidateDragPreview() {
  ...
  document.querySelectorAll('.dragFloatingClone').forEach((node) => node.remove())
  document.removeEventListener('dragover', window.__uniplanCandidateDragMove)
  document.removeEventListener('drag', window.__uniplanCandidateDragMove)
  document.removeEventListener('drop', cleanupCandidateDragPreview)
}

function makeTransparentDragImage(event) {
  try {
    ...
    // Keep the custom floating clone even if the browser ignores transparent drag images.
  }
}
...
function makeCandidateDragPreview(event) {
  ...
  preview.classList.add('dragFloatingClone')
  ...
  preview.style.transform = 'translate(-1200px, -1200px)'
  preview.style.transition = 'none'
  document.body.appendChild(preview)
  source.classList.add('draggingSource')

  const move = (moveEvent) => {
    if (!moveEvent.clientX && !moveEvent.clientY) return
    preview.style.transform = `translate(${moveEvent.clientX - offsetX}px, ${moveEvent.clientY - offsetY}px)`
  }
  window.__uniplanCandidateDragMove = move
  move(event)

  event.dataTransfer.effectAllowed = 'move'
  makeTransparentDragImage(event)
  document.addEventListener('dragover', move)
  document.addEventListener('drag', move)
  document.addEventListener('drop', cleanupCandidateDragPreview, { once: true })
  source.addEventListener('dragend', cleanupCandidateDragPreview, { once: true })
}
