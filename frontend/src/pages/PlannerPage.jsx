...
function makeCandidateDragPreview(event, course, helpers) {
  const source = event.currentTarget
  if (!source || !event.dataTransfer || !course) return
  const detail = helpers.getCourse(course)
  const rect = source.getBoundingClientRect()
  const width = Math.max(190, Math.min(280, rect.width || 230))
  const height = Math.max(86, Math.min(150, rect.height || 100))
  const preview = document.createElement('div')
  preview.className = 'dragPreviewCourseCard candidateDragPreview'
  ...
  dot.className = 'courseTypeDot blue'
  const title = document.createElement('span')
  title.className = 'tileTitle'
  title.textContent = detail.name || '未命名課程'
  const info = document.createElement('span')
  info.className = 'tileMeta'
  info.textContent = [`${helpers.credits(course)} 學分`, detail.time_info || detail.time || '未列時間'].filter(Boolean).join('｜')
  preview.append(dot, title, info)
  document.body.appendChild(preview)
  source.classList.add('draggingSource')
  event.dataTransfer.effectAllowed = 'move'
  event.dataTransfer.setDragImage(preview, Math.min(width / 2, 110), Math.min(height / 2, 54))
                  ...
                  <section className="toolCard candidateRail sideCandidateRail" onDragOver={(e) => e.preventDefault()} onDrop={dropToCandidates}>
                    ...
                    <div className="candidateList liveCandidateList">
                      {filteredCandidates.length ? filteredCandidates.map((c) => {
                        ...
                        return <article key={uid(c)} className="candidateLiveCard" draggable onDragStart={(e) => { makeCandidateDragPreview(e, c, { getCourse, credits }); e.dataTransfer.setData('application/json', JSON.stringify({ source: 'candidate', course: c })) }}>
