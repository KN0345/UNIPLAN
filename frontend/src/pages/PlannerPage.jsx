...
function setCardDragPreview(event) {
  ...
  clone.classList.add('dragPreviewClone')
  ...
  event.dataTransfer.setDragImage(clone, Math.min(rect.width / 2, 80), Math.min(rect.height / 2, 40))
                  ...
                  <section className="toolCard candidateRail sideCandidateRail" onDragOver={(e) => e.preventDefault()} onDrop={dropToCandidates}>
                    ...
                    <div className="candidateList liveCandidateList">
                      {filteredCandidates.length ? filteredCandidates.map((c) => {
                        ...
                        return <article key={uid(c)} className="candidateLiveCard" draggable onDragStart={(e) => { setCardDragPreview(e); e.dataTransfer.setData('application/json', JSON.stringify({ source: 'candidate', course: c })) }}>
