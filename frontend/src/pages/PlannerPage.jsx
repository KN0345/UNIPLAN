              ...
              <div className="plannerLayout">
                ...
                <aside className="plannerTools">
                  ...
                  <section className="toolCard quickActions"><h3>操作</h3>
                    <div className="toolButtons plannerActionGrid"><button onClick={undo} disabled={!history.length}>復原</button><button onClick={redo} disabled={!future.length}>重做</button><button onClick={() => { save(); setShowPlanSchemes(true); }}>儲存快照</button><button onClick={() => exportExcel(plan)}>匯出試算表</button><button onClick={importBackupFile}>匯入文字</button><button onClick={() => download('uniplan.json', JSON.stringify({ plan, candidates, favorites, snapshots, localReviews, tagVotes }, null, 2), 'application/json;charset=utf-8')}>匯出文字</button><button onClick={async () => { await exportPngFromDom(scheduleExportRef.current, activeSemester); }}>匯出課表</button><button onClick={async () => { await exportCleanPng(plan, activeSemester); }}>匯出桌布</button><button onClick={() => exportCalendar(plan, activeSemester)}>匯出行事曆</button><button className="autoPlanOpenButton" disabled={!candidates.length} onClick={() => setShowAutoPlanModal(true)}>智慧排入</button><button className="wideToolButton" onClick={() => setShowManualCourse(true)}>新增自訂課程</button></div></section>
