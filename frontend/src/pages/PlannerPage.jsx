...
function setCandidateDragPreview(event, course, helpers) {
  const source = event.currentTarget
  if (!source || !event.dataTransfer) return

  const courseId = helpers.uid(course)
  document.querySelectorAll(`[data-drag-course-id="${cssEscapeValue(courseId)}"]`).forEach((node) => node.classList.add('draggingSource'))
  document.addEventListener('dragend', clearDragSources, { once: true })

  const rect = source.getBoundingClientRect()
  const detail = helpers.getCourse(course)
  const title = detail.name || '未命名課程'
  const meta = [`${helpers.credits(course)} 學分`, detail.time_info || detail.time || '未列時間'].filter(Boolean).join('｜')
  const previewWidth = Math.max(180, Math.min(260, rect.width || 220))
  const previewHeight = Math.max(84, Math.min(140, rect.height || 96))

  const clone = document.createElement('div')
  clone.className = 'dragPreviewClone dragPreviewCourseCard candidateDragPreview'
  ...
  clone.innerHTML = `
    <b class="courseTypeDot blue"></b>
    <span class="tileTitle"></span>
    <span class="tileMeta"></span>
  `
  clone.querySelector('.tileTitle').textContent = title
  clone.querySelector('.tileMeta').textContent = meta

  document.body.appendChild(clone)
  event.dataTransfer.effectAllowed = 'move'
  event.dataTransfer.setDragImage(clone, Math.min(previewWidth / 2, 100), Math.min(previewHeight / 2, 48))
              ...
              <div className="plannerLayout">
                ...
                <aside className="plannerTools">
                  ...
                  <section className="toolCard quickActions"><h3>操作</h3>
                    <div className="toolButtons plannerActionGrid"><button onClick={undo} disabled={!history.length}>復原</button><button onClick={redo} disabled={!future.length}>重做</button><button onClick={() => { save(); setShowPlanSchemes(true); }}>儲存快照</button><button onClick={() => exportExcel(plan)}>匯出試算表</button><button onClick={importBackupFile}>匯入文字</button><button onClick={() => download('uniplan.json', JSON.stringify({ plan, candidates, favorites, snapshots, localReviews, tagVotes }, null, 2), 'application/json;charset=utf-8')}>匯出文字</button><button onClick={async () => { await exportPngFromDom(scheduleExportRef.current, activeSemester); }}>匯出課表</button><button onClick={async () => { await exportCleanPng(plan, activeSemester); }}>匯出桌布</button><button onClick={() => exportCalendar(plan, activeSemester)}>匯出行事曆</button><button className="autoPlanOpenButton" disabled={!candidates.length} onClick={() => setShowAutoPlanModal(true)}>一鍵排入</button><button className="wideToolButton" onClick={() => setShowManualCourse(true)}>新增自訂課程</button></div></section>
                  <section className="toolCard candidateRail sideCandidateRail" onDragOver={(e) => e.preventDefault()} onDrop={dropToCandidates}>
                    ...
                    <div className="candidateList liveCandidateList">
                      {filteredCandidates.length ? filteredCandidates.map((c) => {
                        ...
                        return <article key={uid(c)} className="candidateLiveCard" data-drag-course-id={uid(c)} draggable onDragStart={(e) => { setCandidateDragPreview(e, c, { getCourse, uid, credits }); e.dataTransfer.setData('application/json', JSON.stringify({ source: 'candidate', course: c })) }}>
        ...
        <div className="manualCourseBackdrop autoPlanBackdrop" onMouseDown={() => setShowAutoPlanModal(false)}>
          <section className="manualCourseModal autoPlanModal" onMouseDown={(e) => e.stopPropagation()}>
            <header className="manualCourseHeader">
              <div><h3>一鍵排入</h3><p>選擇學期範圍後，系統會把暫存區課程自動排入不衝堂的位置。</p></div>
            ...
            <p className="muted autoPlanModalHint">目前將嘗試排入 {selectedAutoPlanSemesters().length} 個學期，暫存區共 {candidates.length} 門課。</p>
            <button className="primaryManualButton" disabled={!candidates.length} onClick={() => { runAutoPlanRange(); setShowAutoPlanModal(false); }}>開始一鍵排入</button>
          </section>
        </div>
      )}
