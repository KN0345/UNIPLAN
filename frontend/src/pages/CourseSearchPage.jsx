...
const GENERIC_DEPARTMENT_ALIASES = [
  ['教育科技學系', '教育科技 教育科技系 教科 教科系 教育科系學系'],
...
export default function CourseSearchPage(props) {
  ...
  useEffect(() => {
    ...
    const initialRender = hasActiveSearch ? 160 : 120
    setRenderLimit(Math.min(sortedFilteredCourses.length, initialRender))
  }, [sortedFilteredCourses.length, query, searchFilters])
  useEffect(() => {
    const node = loadMoreRef.current
    if (!node || renderLimit >= sortedFilteredCourses.length) return undefined
    const loadNext = () => {
      setRenderLimit((value) => Math.min(sortedFilteredCourses.length, value + 120))
    }
    ...
    const onScroll = () => {
      ...
      if (rect.top < window.innerHeight + 520) loadNext()
    }
    window.addEventListener('scroll', onScroll, { passive: true })
    onScroll()
    return () => window.removeEventListener('scroll', onScroll)
  }, [renderLimit, sortedFilteredCourses.length])
              ...
              <div className="searchColumn">
                ...
                {searchTab === 'results' && <><form className="courseSearchToolbar" onSubmit={(e) => { e.preventDefault(); runCourseSearch() }}>
                  <input
                    ...
                    type="text"
                    placeholder="輸入課程名稱或課號"
                    value={searchDraft}
                    onCompositionStart={() => { searchComposingRef.current = true }}
                    onCompositionEnd={(e) => {
                      searchComposingRef.current = false
                      commitSearchDraft(e.currentTarget.value)
                    }}
                    onChange={(e) => {
                      const next = e.target.value
                      setSearchDraft(next)
                      if (!searchComposingRef.current && !e.nativeEvent?.isComposing) {
                        startTransition(() => {
                          setQuery(next)
                        })
                      }
                ...
                <div className="courseFilters">
                  ...
                  <button type="button" onClick={() => { const nextFilters = { department: '全部', requirement: '全部', grade: '全部', weekday: '全部', period: '全部', tag: '全部' }; searchComposingRef.current = false; setSearchDraft(''); setQuery(''); setSearchFilters(nextFilters); setSearchOnlyAvailable(false); setSearchSort('default'); runCourseSearch({ query: '', searchFilters: nextFilters }) }}>重設</button>
                </div>
