    ...
    <section className="searchPage singleColumn">
              <div className="searchColumn">
                ...
                <div className="courseFilters">
                  ...
                  <label>科系<input className="filterTextInput departmentUnifiedInput" list="department-filter-options" value={searchFilters.department === '全部' ? '' : searchFilters.department} onChange={(e) => setSearchFilters((f) => ({ ...f, department: e.target.value.trim() || '全部' }))} placeholder="全部／輸入科系或代碼" /><datalist id="department-filter-options">{departmentOptions.filter((item) => item !== '全部').map((item) => <option key={`dept-${item}`} value={item} />)}</datalist></label>
