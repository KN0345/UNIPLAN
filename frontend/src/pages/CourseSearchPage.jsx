...
function DepartmentFilter({ value, options = [], onChange }) {
  ...
  const filteredOptions = useMemo(() => {
    ...
    const matched = keyword ? list.filter((item) => normalizeOptionText(item).includes(keyword)) : list
...
export default function CourseSearchPage(props) {
  ...
  useEffect(() => {
    ...
    const grow = () => {
      if (cancelled) return
      setRenderLimit((prev) => {
        if (prev >= sortedFilteredCourses.length) return prev
        return Math.min(sortedFilteredCourses.length, prev + 160)
      })
    }
    const schedule = () => {
      if (cancelled) return
      if (typeof window.requestIdleCallback === 'function') {
        window.requestIdleCallback(() => { grow(); schedule() }, { timeout: 350 })
      } else {
        window.setTimeout(() => { grow(); schedule() }, 60)
      }
    }
    schedule()
