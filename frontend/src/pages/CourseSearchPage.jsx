...
const VIRTUAL_CARD_HEIGHT = 210
const VIRTUAL_OVERSCAN = 6

function VirtualCourseList({ courses, renderCourse, empty }) {
  const [scrollTop, setScrollTop] = useState(0)
  const viewportHeight = 720
  const startIndex = Math.max(0, Math.floor(scrollTop / VIRTUAL_CARD_HEIGHT) - VIRTUAL_OVERSCAN)
  const visibleCount = Math.ceil(viewportHeight / VIRTUAL_CARD_HEIGHT) + VIRTUAL_OVERSCAN * 2
  const visibleCourses = courses.slice(startIndex, startIndex + visibleCount)
  if (!courses.length) return empty
  return (
    <div className="courseList virtualCourseList" onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)}>
      <div className="virtualCourseListSpacer" style={{ height: courses.length * VIRTUAL_CARD_HEIGHT }}>
        <div className="virtualCourseListWindow" style={{ transform: `translateY(${startIndex * VIRTUAL_CARD_HEIGHT}px)` }}>
    ...
    <section className="searchPage singleColumn">
              <div className="searchColumn">
                ...
                <div className="courseFilters">
                  ...
                  <label>科系<div className="departmentFilterControl"><select value={searchFilters.department || '全部'} onChange={(e) => setSearchFilters((f) => ({ ...f, department: e.target.value || '全部' }))}><option value="全部">全部</option>{departmentOptions.filter((item) => item !== '全部').map((item) => <option key={`dept-select-${item}`} value={item}>{item}</option>)}</select><input className="filterTextInput" list="department-filter-options" value={searchFilters.department === '全部' ? '' : searchFilters.department} onChange={(e) => setSearchFilters((f) => ({ ...f, department: e.target.value.trim() || '全部' }))} placeholder="或直接輸入科系" /><datalist id="department-filter-options">{departmentOptions.map((item) => <option key={`dept-${item}`} value={item} />)}</datalist></div></label>
