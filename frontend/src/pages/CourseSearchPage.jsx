    ...
    <section className="searchPage singleColumn">
              <div className="searchColumn">
                ...
                <div className="courseFilters">
                  ...
                  <label>科系<input className="filterTextInput" list="department-filter-options" value={searchFilters.department === '全部' ? '' : searchFilters.department} onChange={(e) => setSearchFilters((f) => ({ ...f, department: e.target.value.trim() || '全部' }))} placeholder="輸入或選擇科系" /><datalist id="department-filter-options">{departmentOptions.map((item) => <option key={`dept-${item}`} value={item} />)}</datalist></label>
