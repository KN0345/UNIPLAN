...
async function bulkUpsertCourseRows(sql, courses = []) {
  ...
  await sql`
    ...
    where c.semester_source = i.semester_source
      and (
        (coalesce(i.serial, '') <> '' and coalesce(c.serial, '') = coalesce(i.serial, ''))
        or (
          coalesce(i.serial, '') = ''
...
async function handleAdminPatchCourseImport(request, env, sql) {
  ...
  if (repairAcademicYear114) {
    const repaired = await deleteMisplacedPatchedCourses(sql)
    repairedWrong1141 = repaired.removedWrong1141 || 0
    repairedWrong1142 = repaired.removedWrong1142 || 0
  ...
  return json({
    ...
    patchVersion: '114-academic-year-v2',
  })
}
