...
const PATCHED_COMMON_COURSES = [
  ...
  },
  {
    ...
    "patch_source": "manual-1141-tgaeb-repair",
    "patch_version": "114-academic-year-v3",
    "is_patch": true
  },
  {
    "serial": "1447",
...
function patchedCourseKey(course) {
  ...
  // 開課序號在全校資料中可能重複，補丁 id 不可只用 semester+serial。
...
function normalizePatchedCourse(course) {
  const raw = {
    ...course,
    patch_source: course?.patch_source || 'patched-common-courses',
    patch_version: course?.patch_version || '114-academic-year-v2',
    is_patch: true,
  }
  return mapCourseRow({ id: patchedCourseKey(raw), raw_json: raw, ...raw })
}


async function loadEmbeddedStaticCourses(request) {
  try {
    const url = new URL(request.url)
    const response = await fetch(`${url.origin}/data/courses.json`, { headers: { accept: 'application/json' } })
    if (!response.ok) return []
    const payload = await response.json()
    const list = Array.isArray(payload) ? payload : Array.isArray(payload?.data) ? payload.data : Array.isArray(payload?.courses) ? payload.courses : []
    return list.filter((course) => normalizeCourseCatalogTerm(course?.semester_source || course?.semester).startsWith('114'))
  } catch {
    return []
  }
}

function mergeCourseSources(...sources) {
  const map = new Map()
  for (const source of sources) {
    for (const raw of Array.isArray(source) ? source : []) {
      const normalized = normalizePatchedCourse(raw)
      const key = patchedCourseKey(normalized)
      if (!key) continue
...
function patchedCourseTermSerialPairs(termFilter = '') {
  const target = normalizeCourseCatalogTerm(termFilter)
  return PATCHED_COMMON_COURSES
    .map(normalizePatchedCourse)
    .filter((course) => !target || normalizeCourseCatalogTerm(course.semester_source) === target)
    .map((course) => ({
      semester_source: normalizeCourseCatalogTerm(course.semester_source),
      serial: String(course.serial || '').trim(),
      code: String(course.code || course.course_code || '').trim(),
    }))
    .filter((item) => item.semester_source && item.serial && item.code)
}

async function deleteCoursesByTermSerialPairs(sql, pairs = []) {
  const uniquePairs = []
  const seen = new Set()
  for (const item of pairs) {
    const semester_source = normalizeCourseCatalogTerm(item.semester_source)
    const serial = String(item.serial || '').trim()
    const code = String(item.code || '').trim()
    if (!semester_source || !serial || !code) continue
    const key = `${semester_source}:${serial}:${code}`
    if (seen.has(key)) continue
    seen.add(key)
    uniquePairs.push({ semester_source, serial, code })
  }
  if (!uniquePairs.length) return 0
  const payload = JSON.stringify(uniquePairs)
  const rows = await sql`
    with keys as (
      select * from jsonb_to_recordset(${payload}::jsonb) as x(semester_source text, serial text, code text)
    ), deleted as (
      delete from courses c
      using keys k
      where c.semester_source = k.semester_source
        and coalesce(c.serial, '') = k.serial
        and coalesce(c.code, '') = k.code
      returning 1
    )
...
async function deleteMisplacedPatchedCourses(sql) {
  const patches1141 = patchedCourseTermSerialPairs('1141CLASS')
  const patches1142 = patchedCourseTermSerialPairs('1142CLASS')
  const wrongIn1142 = patches1141.map((item) => ({ ...item, semester_source: '1142CLASS' }))
  const wrongIn1141 = patches1142.map((item) => ({ ...item, semester_source: '1141CLASS' }))
  const removed1142 = await deleteCoursesByTermSerialPairs(sql, wrongIn1142)
  const removed1141 = await deleteCoursesByTermSerialPairs(sql, wrongIn1141)
  return { removedWrong1142: removed1142, removedWrong1141: removed1141 }
}
...
function normalizeImportCourse(course = {}, fallbackSemester = '') {
  return {
    ...
    serial: String(course.serial || course.open_serial || course.openSerial || course.open_course_no || course.openCourseNo || course.開課序號 || course['開課序號'] || '').trim(),
    ...
    raw_json: {
      ...course,
      patch_source: course.patch_source || 'manual-or-official-import',
      patch_version: course.patch_version || null,
      is_patch: Boolean(course.is_patch),
    },
  }
}
...
async function upsertCourseRow(sql, c) {
  ...
  await sql`
    ...
    where semester_source = ${c.semester_source}
      and (
        (coalesce(${c.serial}, '') <> '' and coalesce(serial, '') = ${c.serial})
        or (
          coalesce(${c.serial}, '') = ''
          and coalesce(code, '') = ${c.code}
          and coalesce(department, '') = ${c.department}
          and coalesce(class_name, '') = ${c.class_name}
          and coalesce(teacher, '') = ${c.teacher}
          and coalesce(time_info, '') = ${c.time_info}
          and name = ${c.name}
        )
      )
  `
...
async function bulkUpsertCourseRows(sql, courses = []) {
  ...
  await sql`
    ...
    where c.semester_source = i.semester_source
      and (
        -- 補丁重寫策略：只要同學期同開課序號，就視為同一筆官方課程，先刪除舊資料再加入新資料。
        -- 這能清掉已存在但欄位不完整、department/name/class_name 有舊值的殘留課程。
        (
          coalesce(i.serial, '') <> ''
        ...
        or (
          ...
          and coalesce(c.department, '') = coalesce(i.department, '')
          and coalesce(c.class_name, '') = coalesce(i.class_name, '')
          and coalesce(c.teacher, '') = coalesce(i.teacher, '')
          and coalesce(c.time_info, '') = coalesce(i.time_info, '')
          and c.name = i.name
        )
      )
  `
...
function cloudCourseKey(item) {
  ...
  const term = normalizeCourseCatalogTerm(c.semester_source || c.semester || c.term || c.source_term || c.catalog_term)
  const serial = String(c.serial || c.open_serial || c.openSerial || c.open_course_no || c.openCourseNo || c.開課序號 || c['開課序號'] || '').trim()
  if (serial) return term ? `${term}:${serial}` : serial
...
async function handleAdminPatchCourseImport(request, env, sql) {
  ...
  // 內建缺失課程補丁同時包含 1141CLASS 與 1142CLASS。
  // 舊版會把管理頁目前選到的 semester 當成 fallback / filter，導致 114 上課程被錯誤匯入到 1142CLASS，
  // 或只匯入 114 下。預設應匯入全部補丁，並保留每筆課程自己的 semester_source。
  const requestedSemester = normalizeCourseCatalogTerm(body.semester || body.semester_source || '')
  const onlySemester = Boolean(body.onlySemester || body.only_semester)
  const clearSemester = Boolean(body.clearSemester || body.clear_semester)
  const repair1141 = Boolean(body.repair1141 || body.repair_1141 || body.repairWrongSemester === '1141_TO_1142')
  const completePatch = Boolean(body.completePatch || body.complete_patch || body.action === 'complete')
  const repairAcademicYear114 = Boolean(completePatch || body.repairAcademicYear114 || body.repair_academic_year_114 || body.rebuild114 || body.rebuild_114)
  const targetSemester = repair1141 ? '1141CLASS' : requestedSemester
  const embeddedStaticCourses = await loadEmbeddedStaticCourses(request)
  const sourceCourses = mergeCourseSources(embeddedStaticCourses, PATCHED_COMMON_COURSES)
    .filter((course) => !onlySemester || !targetSemester || normalizeCourseCatalogTerm(course.semester_source) === targetSemester)

  if (!sourceCourses.length) return error('沒有符合條件的內建補丁課程')

  await ensureCourseSchema(sql)
  if (clearSemester) {
    const semestersToClear = [...new Set(sourceCourses.map((course) => normalizeCourseCatalogTerm(course.semester_source)).filter(Boolean))]
    for (const term of semestersToClear) {
      await sql`delete from courses where semester_source = ${term}`
    }
  }

  let repairedWrongSemester = 0
  let repairedWrong1141 = 0
  let repairedWrong1142 = 0
  if (repairAcademicYear114) {
    // 完整補丁：若 Cloudflare 可讀取 /data/courses.json，才允許乾淨重建 1141/1142；
    // 若讀不到靜態課表，避免誤刪整個 Neon，只清理補丁錯置並補寫內建補丁。
    if (embeddedStaticCourses.length) {
      const removed1141 = await sql`delete from courses where semester_source = '1141CLASS' returning 1`
      const removed1142 = await sql`delete from courses where semester_source = '1142CLASS' returning 1`
      repairedWrong1141 = Array.isArray(removed1141) ? removed1141.length : 0
      repairedWrong1142 = Array.isArray(removed1142) ? removed1142.length : 0
      repairedWrongSemester = repairedWrong1141 + repairedWrong1142
    } else {
      repairedWrongSemester = await deleteMisplacedPatchedCourses(sql)
    }
  } else if (repair1141) {
    const repaired = await deleteCoursesByTermSerialPairs(sql, patchedCourseTermSerialPairs('1141CLASS').map((item) => ({ ...item, semester_source: '1142CLASS' })))
    repairedWrong1142 = repaired
    repairedWrongSemester = repaired
  }
  ...
  for (let index = 0; index < sourceCourses.length; index += 1) {
    // 不傳入 requestedSemester 作為 fallback，避免把 1141CLASS 補丁洗成 1142CLASS。
    const course = normalizeImportCourse(sourceCourses[index], '')
    if (!course.semester_source || !course.name) {
      errors.push({ row: index + 1, message: '缺少學期或課程名稱' })
      continue
    }
  ...
  return json({
    ...
    errors,
    bySemester,
    repairedWrongSemester,
    repairedWrong1141,
    repairedWrong1142,
    source: 'patched-common-courses',
    patchVersion: completePatch ? 'complete-114-academic-year-v6-all-college-common-required' : '114-academic-year-v3',
    completePatch,
    embeddedStaticCount: embeddedStaticCourses.length,
    collegeRequiredClassification: true,
  })
}
  ...
  try {
    ...
    rows = await sql`
      ...
      WHERE (${semester} = '' OR semester_source = ${semester})
        ...
        AND (
          ${keyword} = ''
          OR lower(coalesce(name, '')) LIKE lower(${'%' + keyword + '%'})
          OR lower(coalesce(code, '')) LIKE lower(${'%' + keyword + '%'})
          OR lower(coalesce(serial, '')) LIKE lower(${'%' + keyword + '%'})
          OR lower(coalesce(teacher, '')) LIKE lower(${'%' + keyword + '%'})
          OR lower(coalesce(department, '')) LIKE lower(${'%' + keyword + '%'})
...
function normalizeFunctionPath(request, params) {
  const fromParams = `/${(params?.path || []).join('/')}`.replace(/\/+/g, '/')
  if (fromParams && fromParams !== '/') return fromParams

  // Cloudflare Pages normally strips /api for functions/api/[[path]].js.
  // This fallback makes /api/user/data and /user/data resolve to the same internal route.
...
export async function onRequest(context) {
  ...
  try {
    const path = normalizeFunctionPath(request, params)
    ...
    if (method === 'HEAD' && path === '/user/data') return new Response(null, { headers: jsonHeaders })
    ...
    if (method === 'HEAD' && path === '/user/welcome') return new Response(null, { headers: jsonHeaders })
