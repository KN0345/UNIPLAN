...
function normalizeImportCourse(course = {}, fallbackSemester = '') {
  return {
    ...
    serial: String(course.serial || course.open_serial || course.opening_serial || course.openSerial || course.openCourseNo || course.openCourseNumber || course.course_serial || course.sequence || course.seq || course.no || course.序號 || course['序號'] || course.開課序號 || course['開課序號'] || '').trim(),
  ...
  try {
    ...
    rows = await sql`
      ...
      WHERE (${semester} = '' OR semester_source = ${semester})
        ...
        AND (${keyword} = '' OR (
          coalesce(name, '') || ' ' || coalesce(code, '') || ' ' || coalesce(serial, '') || ' ' ||
          coalesce(teacher, '') || ' ' || coalesce(department, '') || ' ' || coalesce(class_name, '') || ' ' || coalesce(notes, '')
        ) ILIKE ${'%' + keyword + '%'})
