...
function normalizeImportCourse(course = {}, fallbackSemester = '') {
  return {
    ...
    serial: String(course.serial || course.open_serial || course.openCourseNo || course.open_course_no || course.openCourseSerial || course.open_course_serial || course.開課序號 || course['開課序號'] || course['開課序號(教學計畫表)'] || '').trim(),
    code: String(course.code || course.course_code || course.courseCode || course.course_id || course.subject_code || course.科目編號 || course['科目編號'] || course.課號 || course['課號'] || '').trim(),
    ...
    class_name: String(course.class_name || course.className || course.class || course.section || course.班別 || course['班別'] || course.班級 || course['班級'] || '').trim(),
